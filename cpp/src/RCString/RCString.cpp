...
RCString &RCString::Concat(const RCString &o_)
{
    ...
    // state is still don't matter if the string is changed because it will be 
    // changed into the excat same string (changing him to himeself).
    ...
    // Copy the second string as appended to the first one (inside the new buffer).
    size_t j = 0;
