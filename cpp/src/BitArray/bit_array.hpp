...
void ShiftRightOp(unsigned char *bit_array, size_t bits_num, size_t shift)
{
    ...
    if (((byte_index + 1) < bit_array_length) &&
                                            (0 != bit_index)) 
    {
        transform(bit_array + byte_index + 1,
                       bit_array + bit_array_length,
                       bit_array + byte_index,
                       bit_array + byte_index,
    ...
    if (0 != byte_index) 
    {
        copy(bit_array + byte_index, bit_array + bit_array_length,
                        bit_array);
    }
...
void ShiftLeftOp(unsigned char *bit_array, size_t bits_num, size_t shift)
{
    ...
    if (0 != byte_index) // 1 or more bytes should be turned into zeros
    {
        copy_backward(bit_array,
                      bit_array + (bit_array_length - byte_index),
                      bit_array + bit_array_length);
    }

    if (((byte_index + 1) < bit_array_length) && (0 != bit_index)) 
    {
        transform(bit_array + byte_index,
                  bit_array + bit_array_length,
                  bit_array + byte_index,
                  ShiftLeft(bit_index));
    }
