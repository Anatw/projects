                                       /*
 * This it the original busexmp.c file with changes - this buse writes to a file
 * (a memory in disk) and not to a malloc (heap) - as the original version does.
 *
 * "file.dat" must be created in the buse folder.
 *
 * Created: Septermber 2020.
 *
 *
 *
...
static int xmp_read(void* buf, u_int32_t len, u_int64_t offset, void* userdata)
{
    ...
    if (fp)
    {
        // Go to the requested offset inside the file, and read from it the
        // requested length:
    ...
    {
        perror("Inside xmp_read(): ERROR in fopen");
        return 1;
    }
                     ...
                     void* userdata)
{
    ...
    if (fp)
    {
        // Go to the requested offset inside the file, and write to it:
        fseek(fp, offset, SEEK_SET);
