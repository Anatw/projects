...
static size_t HasLeftChild(size_t index)
{
    return (0 < GetLeftChildIndex(index) ? 1 : 0);
}

static size_t HasRightChild(size_t index)
{
    return (0 < GetRightChildIndex(index) ? 1 : 0);
}
...
/* compare root node with both children and swap with tje smaller one. Kepp
   checking and swapping until reaching a node with bigger value. */
static void Heapify(heap_t *heap)
{
    size_t root = 0;
    size_t smaller_child_index = 0;
    assert(heap);

    root = VectorSize(heap->vector) - 1;
    ...
    while (HasLeftChild(root))
    {
        ...
        smaller_child_index = GetLeftChildIndex(root);
        if (HasRightChild(root) &&
            RightChild(heap->vector, root) < LeftChild(heap->vector, root))
        {
            smaller_child_index = GetRightChildIndex(root);
        }

        /* if index is smaller then smaller-child-index - do nothing, else - 
           swap between them: */
        if (0 < (heap->cmp_func(VectorGetElement(heap->vector, root),
                                VectorGetElement(heap->vector, smaller_child_index))))
        {
            break;
        }
        else
        {
            SwapData(heap->vector, root, smaller_child_index);
            root = smaller_child_index;
        }
    }
}
...
int HeapPush(heap_t *heap, const void *data)
{
    ...
    size_t currents_parent = 0;
    void *currents_parent_data = NULL;
    assert(heap);
    ...
    currents_parent = (current - 1) / 2;
    current_data = VectorGetElement(heap->vector, current);
	currents_parent_data = VectorGetElement(heap->vector, currents_parent);

    /* while we haven't reached the root AND the child has greater value than
       parrent - keep swapping: */
	while (ZERO < heap->cmp_func(current_data, currents_parent_data))
	{
		current_data = VectorGetElement(heap->vector, current);
		currents_parent_data = VectorGetElement(heap->vector, currents_parent);
		...
		currents_parent = GetParentIndex(currents_parent);
                 ...
                 const void *param)
{
    ...
    if (0 == cond_func(VectorGetElement(heap->vector, i) ,param))
    {
        return_data = VectorGetElement(heap->vector, i);

        VectorSetElement(heap->vector, i,
                         VectorGetElement(heap->vector, (HeapSize(heap) - 1)));
