...
#define IS_CHILDLESS(current)                                                  \
    (!IS_LEFT_CHILD(current) && !IS_RIGHT_CHILD(current))
...
void BSTPushAllElementsToStack(bst_t* tree, stack_t* stack)
{
    ...
    for (iterator = BSTBegin(tree); iterator != BSTEnd(tree);
...
void BSTRemove(bst_iter_t iterator)
{
    ...
    else if (iterator->nodes[RIGHT] && iterator->nodes[LEFT])
    {
        ...
        else
        {
            ...
            substitute_node->parent->nodes[RIGHT] =
                substitute_node->nodes[LEFT];
...
bst_iter_t BSTFind(bst_t* tree, const void* data)
{
    ...
    {
        if (1 ==
            tree->cmp_func(new_data, BSTGetData(current_node), tree->param))
        {
            direction = RIGHT;
        }
        else
...
void PreOrder(bst_iter_t root)
{
    ...
    StackPush(root->data, stack);

    while (!StackIsEmpty(stack))
    {
        node = StackPeek(stack);
        StackPop(stack);
        printf("%d\n", node->data);

        if (node->nodes[RIGHT])
        {
            StackPush(node->nodes[RIGHT]->data, stack);
        }
        if (node->nodes[RIGHT])
        {
            StackPush(node->nodes[LEFT]->data, stack);
        }
    }
}
