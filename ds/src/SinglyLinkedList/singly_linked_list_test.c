...
int main()
{
	...
	int data4 = 6;
	int *p_data = &data;
	const int *p_data2 = &data2;
	iter_t node1 = SLLInsert(SLLEnd(llist), p_data);
	iter_t node2 = NULL;
	iter_t node3 = NULL;
	iter_t node4 = NULL;
	...
	printf("\nnumber of members in the list is: %ld\n\n", SLLCount(llist));
		...
		for(iterator = SLLBegin(llist); !SLLIsSameIter(iterator,
				SLLEnd(llist)); iterator = SLLNext(iterator))
	{
		printf("the next node data is: %d\n", *(int *)SLLGetData(iterator));
	}
	
	SLLForEach(SLLBegin(llist), SLLEnd(llist), &param, AddParam);
	printf("\nafter 'ForEach': add 1:\n");
