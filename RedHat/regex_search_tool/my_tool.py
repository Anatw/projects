...
class SearchInFile:
    ...
    def parse_arguments():
        ...
        parser.add_argument('-f', '--file', required=True, nargs='+',
                            action='append',
                            help='<Required> file to search inside for '
                                 'a string')
        parser.add_argument('-s', '--string', type=str, required=True,
                            help='<Required> string to search for inside '
                                 'file/s')
    ...
    def search_string(file_name: str, string_to_search: str) -> list:
        """ Search the file for the given string and return information
        about each string match. The tool will recognise more than a
        single match pre line, as long as they don't overlap """
