/*******************************************************************************
Multy threaded counting sort & Shuffle and sort dictionary file
...
/* Function to print a massage to the terminal: */
...
void Vocabulary(void)
{
    ...
    /* calculate amount of words (lines) in dictionary */
    while (NULL != fgets(buffer,WORD_SIZE,fp))
    {
        ++amount_of_lines;
    }
    ...
    arr_lenght = malloc((sizeof(int) * NUM_DICTIONARY) * amount_of_lines);

    /* set all lengths in buff */
    for (i = 0; fgets(buffer, WORD_SIZE, fp) != NULL; ++i)
    {
    ...
    MultiplyBuff(arr_lenght, amount_of_lines);
    /* Divide work to threads: */
    amount_of_lines *= NUM_DICTIONARY;
    work_for_thread = amount_of_lines / N_THREADS;
    remainder = amount_of_lines - work_for_thread * N_THREADS;
    ...
    for (i = 0; i < N_THREADS-1; ++i)
    {
        ...
        data[i].amount_of_lines = work_for_thread;
        data[i].count_arr = count_arr[i];
        data[i].min = min;

        RETURN_BAD((0 != pthread_create(&jobs[i], NULL, SortCountPart,&data[i])),1, "FAIL pthread_create \n")
    }

    data[i].start_read = arr_lenght + i * work_for_thread;
    data[i].amount_of_lines = work_for_thread + remainder;
    data[i].count_arr = count_arr[i];
    data[i].min = min;
    ...
    {
        size_t num = 0;
        RETURN_BAD((0 != pthread_join(jobs[i] ,(void**)&num)), 1, "ERROR in pthread_create\n")
    }
    ...
    /*PrintBuff(arr_lenght, amount_of_lines); */
    printf("ANSWER %d\n", answer);
...
static void *SortCountPart(void *param)
{
    ...
    size_t i = 0;
    size_t counter = 0;
    for (i = 0; i < data->amount_of_lines; ++i)
    {
