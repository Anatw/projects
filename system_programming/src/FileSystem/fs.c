...
void GetGroupDescriptor(int fd, struct ext2_group_desc *group_des)
{
    ...
    lseek(fd, (BLOCK_SIZE + BLOCK_SIZE), SEEK_SET);
               ...
               struct ext2_inode *ret_inode)
{
    ...
    printf("the i_block of inode %d is: %d\n\n\n", inode_number, ret_inode->i_block);
                     ...
                     const struct ext2_group_desc *group_descriptor)
{
    ...
    if (S_ISDIR(inode->i_mode))
    {
        ...
        lseek(fd, BLOCK_OFFSET(inode->i_block[3]), SEEK_SET);
...
void SuperBlockTest()
{
    ...
    inode from the beginning of the inode table: */
