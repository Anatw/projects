/****************************************************************************
* File Name: variable_size_memory_allocator.c								*
* Writer: Lior Cohen														*
* Date: 24/03/2020														    *
* Purpose: The functions for the variable size memory allocation			*
* Code reviewer: Esti Binder												*
*****************************************************************************/
...
vsa_t *VSAInit(void *memory, size_t seg_size)
{
	...
	end_vsa->valid = seg_size - (seg_size % WORDSIZE);
	#endif /* DO_DEBUG */
...
size_t VSALargestBlockAvailable(vsa_t *pool)
{
	...
	while (MAX_SIZE != current_block->block_size)
	{
		...
		/* If it's not free than it's negative */
		if (0 > current_block->block_size)
		{
			offset += sizeof(vsa_t) + (current_block->block_size * -1);
		}
		else
		{
			offset += sizeof(vsa_t) + current_block->block_size;
		}
